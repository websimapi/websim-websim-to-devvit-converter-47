...
// 7. Configure Playtest Subreddit
console.log('\\n📝 Configuring Test Subreddit...');
console.log('   Devvit requires a subreddit to playtest your game (must be <200 members & you must be mod).');
console.log('   If you don\\'t have one, go to https://www.reddit.com/subreddits/create');

const sub = await ask('   Enter subreddit name for testing (leave empty to skip): ');
if (sub.trim()) {
    const s = sub.trim().replace(/^r\\//, '');
    try {
        console.log(\`   Installing app to r/\${s}...\`);
        execSync(\`npx devvit install \${s}\`, { stdio: 'inherit' });
        
        // Save to .env for npm run dev
        let envContent = '';
        if (fs.existsSync('.env')) envContent = fs.readFileSync('.env', 'utf8');
        
        if (!envContent.includes('DEVVIT_SUBREDDIT=')) {
            fs.appendFileSync('.env', \`\\nDEVVIT_SUBREDDIT=\${s}\\n\`);
            console.log('   ✅ Subreddit configured in .env for "npm run dev"');
        } else {
            console.log('   ℹ️  .env already has DEVVIT_SUBREDDIT configured.');
        }
    } catch(e) {
        console.warn('   ⚠️  Could not install to that subreddit. Ensure it exists and you are a moderator.');
        console.warn('       You can try manual install later: npx devvit install <subreddit>');
    }
} else {
    console.log('   ⚠️  Skipping configuration. "npm run dev" might fail if it can\\'t find a default subreddit.');
    console.log('       Fix later by running: npx devvit install <your-subreddit>');
}
